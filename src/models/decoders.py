...
class GlobalAttentionDecoder(nn.Module):
    ...
    def forward(self, decoder_input, hidden, encoder_outputs, mask=None):
        ...
        attn_scores = encoder_outputs.permute(1, 0, 2).bmm(h.permute(
            1, 2, 0))  # dot attention，(batch_size,seq_len,1)
        ...
        output = torch.cat([context.permute(2, 0, 1), h],
...
class BahdanauAttentionDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        dot_vector = torch.randn(
            (self.hidden_size, ), dtype=torch.float32,
            device=device) / self.hidden_size
        self.dot_vector = nn.Parameter(dot_vector)
        self.lstm = nn.LSTM(
            input_size=embedding_dim + self.hidden_size,
    ...
    def forward(self, decoder_input, hidden, encoder_outputs, mask=None):
        ...
        context = torch.bmm(
            encoder_outputs.permute(1, 2,
                                    0), attn_weights.unsqueeze(2)).permute(
                                        2, 0, 1)  # (1,batch_size,hidden_size)
        output, hidden = self.lstm(
